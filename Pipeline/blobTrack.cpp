    ...
    while ( camera.IsGrabbing())
    {
      ...
      for(;! done;)
      {
          ...
          if (guiQueue.try_pop(pData))
          {
              ...
              cout << "Execution Rate: " << fixed << fps << " fps." << endl;
                       ...
                       tbb::concurrent_bounded_queue<ProcessingChainData *> &guiQueue)
{
    ...
    tbb::parallel_pipeline(4, // TBB NOTE: (recommendation) NumberOfFilters
                                                                  ...
                                                                  [&](ProcessingChainData *pData)->ProcessingChainData*
                          {
                              ...
                              inRange(pData->red, cv::Scalar(0, 160,160), cv::Scalar(30, 255,255), lower_red_hue_range);
                              inRange(pData->red, cv::Scalar(150, 160,160), cv::Scalar(185, 255,255), upper_red_hue_range);
                                                                ...
                                                                [&](ProcessingChainData *pData)->ProcessingChainData*
                          {
                            ...
                            blur(pData->blue,pData->blue,Size(3,3));

                            inRange(pData->blue, cv::Scalar(90,205,255), cv::Scalar(102,255,255),pData->blue);

                            dilateMat(3,pData->blue);

                            pData->blueArea = pData->img+pData->img*.1;
                            cvtColor(pData->blueArea,pData->blueArea,CV_BGR2HSV);

                            inRange(pData->blueArea, cv::Scalar(70,40,40), cv::Scalar(120, 255,255),pData->blueArea);

                            GaussianBlur(pData->blueArea,pData->blueArea,Size(3,3),0,0);

                            vector<vector<Point> > contactContours;
                            vector<vector<Point> > bigContour;
                            vector<Vec4i> hierarchy;

                            findContours(pData->blue,contactContours,hierarchy,CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE, Point(0,0) );

                            findContours(pData->blueArea,bigContour,hierarchy,CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE,Point(0,0) );
                            cvtColor(pData->blue,pData->blue,CV_GRAY2BGR);
